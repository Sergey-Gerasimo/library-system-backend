...
run_test() {
    local endpoint=$1
    local name=$2
    local report_file="report_${name}_$(date +%Y%m%d_%H%M%S).txt"
...
wait
echo "Все тесты завершены. Отчеты сохранены в файлы report_*.txt"

# Вывод статистики
echo -e "\nКраткая статистика:"
grep "Requests per second" report_*.txt
grep "Time per request" report_*.txt
